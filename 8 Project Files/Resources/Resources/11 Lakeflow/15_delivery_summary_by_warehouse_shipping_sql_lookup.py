...
target_table = "course_training_catalog.bronze_ecommerce_orders.gold_delivery_summary_by_warehouse_shipping_sql_lookup"
...
SELECT
    ...
    CAST(ROUND(SUM(net_amount), 2) AS DOUBLE) AS total_sales_amount,
    CAST(ROUND(AVG(delivery_days), 2) AS DOUBLE) AS avg_delivery_days,
    CAST(SUM(CASE WHEN delivery_status = 'late' THEN 1 ELSE 0 END) AS BIGINT) AS late_orders,
    CAST(SUM(CASE WHEN delivery_status = 'on_time' THEN 1 ELSE 0 END) AS BIGINT) AS on_time_orders,
    CAST(
        ROUND(
