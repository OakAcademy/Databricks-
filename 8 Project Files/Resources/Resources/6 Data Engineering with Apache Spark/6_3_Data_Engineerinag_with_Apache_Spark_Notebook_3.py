...
# MAGIC %sql
# MAGIC SELECT * FROM course_training_catalog.silver_olist.customers;
...
# MAGIC %sql
# MAGIC SELECT * FROM course_training_catalog.silver_olist.sellers;
...
# MAGIC     SUM(seller_count) OVER(ORDER BY seller_count DESC, city_display ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cum_sellers
# MAGIC   FROM
# MAGIC     city_counts
# MAGIC )
# MAGIC SELECT
...
# MAGIC %sql
# MAGIC SELECT * FROM course_training_catalog.gold_olist.product_category_summary;
...
# MAGIC %sql
# MAGIC SELECT * FROM course_training_catalog.silver_olist.order_items;
...
# MAGIC %md
# MAGIC **2.1. Payment Method Distribution**
# MAGIC
# MAGIC * Analyze the share of card, boleto, voucher, or debit payments.
# MAGIC * Visualize as a pie or bar chart to show popular payment preferences.
# MAGIC
# MAGIC **2.2. Average Number of Installments**
# MAGIC
# MAGIC * Measure customers’ average installment count to understand payment flexibility and behavior.
# MAGIC
# MAGIC **2.3. Average Installment Value**
# MAGIC
# MAGIC * Use the previously calculated `payment_per_installment` metric to examine credit card usage patterns.
# MAGIC
# MAGIC **2.4. Time-Based Payment Analysis**
# MAGIC
# MAGIC * Examine payments by week or month to uncover seasonality in purchasing behavior.
# MAGIC
# MAGIC **2.5. Total and Average Payment Volume**
# MAGIC
# MAGIC * `SUM(payment_value)` → total transaction volume
# MAGIC * `AVG(payment_value)` → average order payment amount
# MAGIC
# MAGIC ---
...
# MAGIC %sql
# MAGIC SELECT * FROM course_training_catalog.silver_olist.order_reviews;
